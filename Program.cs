...
class Program
{
	...
	{
		string pivDirPath = "./samples/raw";

		foreach (var pivFilePath in Directory.GetFileSystemEntries(pivDirPath, args.Length > 0 ? args[0] : "*.piv"))
		{
			Console.Error.WriteLine($"\x1b[92m{pivFilePath}\x1b[0m");
			try
			{
				ParsePivFile(File.OpenRead(pivFilePath));
			}
			catch (Exception ex)
			{
	...
	static void ParsePivFile(Stream stream)
	{
		...
		{
			stream.Position = 0;
			ParsePivFile(new ZLibStream(stream, CompressionMode.Decompress));
		}
